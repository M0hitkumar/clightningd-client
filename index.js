...
const debug = require('debug')('clightningd-client');
...
class LightningClient extends EventEmitter {
    constructor(rpcPath=defaultRpcPath) {
        ...
        if (!fExists(rpcPath) || !fStat(rpcPath).isSocket()) {
          ...
          // main data directory provided, default to using the brocoin mainnet subdirectory
          // to be removed in v0.2.0
          else if (fExists(rpcPath, 'brocoin', 'lightning-rpc')) {
            console.error(`WARN: ${rpcPath}/lightning-rpc is missing, using the brocoin mainnet subdirectory at ${rpcPath}/brocoin instead.`)
            ...
            rpcPath = path.join(rpcPath, 'brocoin', 'lightning-rpc')
          }
        }
